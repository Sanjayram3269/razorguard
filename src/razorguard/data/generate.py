...
from razorguard.config import ACCOUNTS_PATH, CHARGEBACKS_PATH, GENERATED_DATA, RANDOM_SEED, TRANSACTIONS_PATH
...
def generate_accounts(rng: np.random.Generator, n_accounts: int) -> pd.DataFrame:
    created = pd.Timestamp("2025-01-01") + pd.to_timedelta(rng.integers(0, 420, n_accounts), unit="D")
    accounts = pd.DataFrame({
        "account_id": [f"A{i:06d}" for i in range(n_accounts)],
        "created_at": created,
        "home_country": rng.choice(COUNTRIES, n_accounts, p=[0.55, 0.15, 0.10, 0.08, 0.07, 0.05]),
        "device_id": [f"D{i:06d}" for i in rng.integers(0, int(n_accounts * 0.82), n_accounts)],
        "account_segment": rng.choice(["new", "standard", "premium"], n_accounts, p=[0.18, 0.67, 0.15]),
    })
    ...
    coordinated_ids = rng.choice([f"CD{i:04d}" for i in range(max(1, coordinated.sum() // 3))], size=coordinated.sum())
    accounts.loc[coordinated, "device_id"] = coordinated_ids

    shared = accounts["scenario"].eq("shared_infrastructure")
    shared_ids = rng.choice([f"SH{i:04d}" for i in range(max(1, shared.sum() // 4))], size=shared.sum())
    accounts.loc[shared, "device_id"] = shared_ids
    return accounts


def _transaction_times(rng: np.random.Generator, accounts: pd.DataFrame, account_idx: np.ndarray, n_transactions: int) -> tuple[pd.DatetimeIndex, np.ndarray]:
    """Create a chronological event stream with explicit burst clustering."""
    start = pd.Timestamp("2026-01-01")
    horizon_seconds = 180 * 24 * 60 * 60
    raw_seconds = rng.integers(0, horizon_seconds, n_transactions)
    scenarios = accounts.iloc[account_idx]["scenario"].to_numpy()
    burst_indices = np.flatnonzero(scenarios == "burst")

    # Give burst accounts several events around account-specific centers. The
    # number of burst events is intentionally modest so bursts remain a signal,
    # not a deterministic label.
    if len(burst_indices):
        centers = rng.integers(0, horizon_seconds, len(burst_indices))
        raw_seconds[burst_indices] = np.clip(centers + rng.integers(-15 * 60, 15 * 60 + 1, len(burst_indices)), 0, horizon_seconds - 1)

    timestamps = start + pd.to_timedelta(raw_seconds, unit="s")
    order = np.argsort(np.asarray(timestamps, dtype="datetime64[ns]"), kind="stable")
    return pd.DatetimeIndex(timestamps[order]), order


def generate_transactions(rng: np.random.Generator, accounts: pd.DataFrame, n_transactions: int) -> pd.DataFrame:
    ...
    # Compromise is account-specific. Each compromised account has its own
    # hidden transition timestamp; the model only sees resulting behavior.
    post_compromise = scenario == "compromised"
    compromise_at = accounts_used["compromise_at"].to_numpy()
    valid_compromise = pd.notna(compromise_at)
    post_compromise &= valid_compromise & (timestamps.to_numpy() >= compromise_at)
    amount *= np.where(post_compromise, accounts_used["compromise_multiplier"].to_numpy(dtype=float), 1.0)

    burst = scenario == "burst"
    amount *= np.where(burst, rng.uniform(1.15, 2.0, n_transactions), 1.0)
    amount = np.clip(amount, 20, 25_000)

    ip_country = rng.choice(COUNTRIES, n_transactions, p=[0.62, 0.12, 0.09, 0.07, 0.06, 0.04])
    ...
    payment_method = rng.choice(PAYMENT_METHODS, n_transactions, p=[0.48, 0.32, 0.12, 0.08])
    category = rng.choice(MERCHANT_CATEGORIES, n_transactions, p=[0.27, 0.20, 0.24, 0.12, 0.17])

    hour = pd.Series(timestamps).dt.hour.to_numpy()
    night = ((hour <= 5) | (hour >= 23)).astype(int)
    account_age_days = (pd.Series(timestamps).dt.normalize().to_numpy() - accounts_used["created_at"].dt.normalize().to_numpy()) / np.timedelta64(1, "D")
    ...
    shared_device = pd.Series(accounts_used["device_id"]).duplicated(keep=False).to_numpy()
    ...
    is_chargeback = rng.random(n_transactions) < sigmoid(risk_logit)

    device_id = accounts_used["device_id"].to_numpy().copy()
    device_id[post_compromise] = accounts_used.loc[post_compromise, "secondary_device_id"].to_numpy()
...
def generate_chargebacks(rng: np.random.Generator, transactions: pd.DataFrame) -> pd.DataFrame:
    cb = transactions.loc[transactions["is_chargeback"] == 1, ["transaction_id", "timestamp"]].copy()
    cb["chargeback_at"] = cb["timestamp"] + pd.to_timedelta(rng.integers(3, 35, len(cb)), unit="D")
    cb["reason_code"] = rng.choice(["fraud", "card_not_present", "duplicate", "service_not_received"], len(cb), p=[0.52, 0.24, 0.08, 0.16])
